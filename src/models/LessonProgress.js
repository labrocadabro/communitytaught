...
lessonProgressSchema.statics.toggleWatched = async function(lessonId, userId) {
	...
	if (progress.watched){
		...
		let nextLessons = await Lesson.find({_id: {$gt: lessonId}, videoId: { $ne: "" }}).lean().sort({_id: -1});
		if (nextLessons.length) {
			...
			const watchedLessonIndex = nextLessons.findIndex(lesson => lesson.watched);
		...
		let prevLessons = await Lesson.find({_id: {$lt: lessonId}, videoId: { $ne: "" }}).lean().sort({_id: 1});
