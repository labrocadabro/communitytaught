...
import LessonProgress from "../lesson/models/LessonProgress.js";
...
export const addEditHomework = async (req, res) => {
	...
	try {
		...
		for (let i = 0; i < hwDesc.length; i++) {
			...
			const hwItem = await HomeworkItem.findByIdAndUpdate(
				hwId[i] || mongoose.Types.ObjectId(),
				item,
		...
		for (let i = 0; i < pwDesc.length; i++) {
			...
			const pwItem = await HomeworkExtra.findByIdAndUpdate(
				pwId[i] || mongoose.Types.ObjectId(),
				extra,
				{ upsert: true, new: true }
			);
		...
		req.session.flash = {
			type: "success",
			message: [`Homework ${!!req.params.id ? "updated" : "added"}`],
		};
	} catch (err) {
		console.log(err);
		req.session.flash = {
	...
	try {
		...
		if (req.body.classesWatched || req.body.classesCheckedin) {
			...
			Object.keys(data).forEach((key) =>
				classes.add(Number(key.split("-")[0].slice(2)))
			);
			...
			if (req.body.classesWatched) {
				...
				const user = await User.findByIdAndUpdate(req.user.id, {
					currentClass: lessonData[currentClass]
						? lessonData[currentClass]
						: null,
				});
			}
		}
